...
var HomeworkCompletionType = /* @__PURE__ */ ((HomeworkCompletionType2) => {
  HomeworkCompletionType2[HomeworkCompletionType2["INDIVIDUA"] = 1] = "INDIVIDUA";
  HomeworkCompletionType2[HomeworkCompletionType2["GRUOP"] = 2] = "GRUOP";
...
var LEARN_NOTIFICATION_LIST = (courseID, courseType) => courseType === "student" /* STUDENT */ ? `${LEARN_PREFIX}/b/wlxt/kcgg/wlkc_ggb/student/kcggListXs?wlkcid=${courseID}&size=${MAX_SIZE}` : `${LEARN_PREFIX}/b/wlxt/kcgg/wlkc_ggb/teacher/kcggList?wlkcid=${courseID}&size=${MAX_SIZE}`;
...
var LEARN_HOMEWORK_LIST_SOURCE = (courseID) => [
  {
...
var LEARN_HOMEWORK_LIST_NEW = (courseID) => `${LEARN_PREFIX}/b/wlxt/kczy/zy/student/index/zyListWj?wlkcid=${courseID}&size=${MAX_SIZE}`;
var LEARN_HOMEWORK_LIST_SUBMITTED = (courseID) => `${LEARN_PREFIX}/b/wlxt/kczy/zy/student/index/zyListYjwg?wlkcid=${courseID}&size=${MAX_SIZE}`;
var LEARN_HOMEWORK_LIST_GRADED = (courseID) => `${LEARN_PREFIX}/b/wlxt/kczy/zy/student/index/zyListYpg?wlkcid=${courseID}&size=${MAX_SIZE}`;
...
var LEARN_HOMEWORK_LIST_TEACHER = (courseID) => `${LEARN_PREFIX}/b/wlxt/kczy/zy/teacher/index/pageList?wlkcid=${courseID}&size=${MAX_SIZE}`;
...
var LEARN_WEBSITE_LANGUAGE = (lang) => `https://learn.tsinghua.edu.cn/f/wlxt/common/language?websiteShowLanguage=${WebsiteShowLanguage[lang]}`;
...
var LEARN_FAVORITE_OR_COMMENT_LIST_FORM_DATA = (courseID) => {
  const form = new FormData();
...
var Learn2018Helper = class {
  ...
  async getNotificationList(courseID, courseType = "student" /* STUDENT */) {
    var _a, _b, _c, _d;
    const json = await (await __privateGet(this, _myFetchWithToken).call(this, LEARN_NOTIFICATION_LIST(courseID, courseType))).json();
  ...
  async getHomeworkList(courseID, courseType = "student" /* STUDENT */) {
    ...
    if (courseType === "teacher" /* TEACHER */) {
      const json = await (await __privateGet(this, _myFetchWithToken).call(this, LEARN_HOMEWORK_LIST_TEACHER(courseID))).json();
      if (json.result !== "success") {
        return Promise.reject({
          reason: "invalid response" /* INVALID_RESPONSE */,
          extra: json
      ...
      return Promise.all(
        LEARN_HOMEWORK_LIST_SOURCE(courseID).map((s) => this.getHomeworkListAtUrl(s.url, s.status))
  ...
  async getFavorites(courseID, type) {
    ...
    const json = await (await __privateGet(this, _myFetchWithToken).call(this, LEARN_FAVORITE_LIST(type), {
      method: "POST",
      body: LEARN_FAVORITE_OR_COMMENT_LIST_FORM_DATA(courseID)
    })).json();
    if (json.result !== "success") {
      return Promise.reject({
        reason: "invalid response" /* INVALID_RESPONSE */,
  ...
  async getHomeworkListAtUrl(url, status) {
    var _a, _b;
    const json = await (await __privateGet(this, _myFetchWithToken).call(this, url)).json();
    if (json.result !== "success") {
      return Promise.reject({
        reason: "invalid response" /* INVALID_RESPONSE */,
        extra: json
  ...
  async parseHomeworkDetail(courseID, id) {
    const response = await __privateGet(this, _myFetchWithToken).call(this, LEARN_HOMEWORK_DETAIL(courseID, id));
    const result = $(await response.text());
