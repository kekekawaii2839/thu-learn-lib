...
function makeFetch(jar) {
  return (0, _fetchCookie.default)(fetch, jar);
}
...
class Learn2018Helper {
  ...
  async getCourseList(semesterID, courseType = _types.CourseType.STUDENT) {
  ...
  async getFavorites(courseID, type) {
    ...
    return result.map(e => {
      ...
      return {
        ...
        time: type === _types.ContentType.DISCUSSION || type === _types.ContentType.QUESTION ? new Date(e.tlsj) : new Date(e.ywsj),
        state: e.ywzt,
